...
# TO DO
# - make sure I have all prompts from old draft (CS, IQOQ, traversal)
# - sort datasets
...
# - sort IQ/OQ generation - make sure CS is incorporated - get ratio from write up draft
...
IQ_prompt = Path("data/prompts/7b_IQ_prompt_CS.txt").read_text()
OQ_prompt = Path("data/prompts/7b_OQ_prompt_CS.txt").read_text()
...
def generate_iqoq( # i need to get the ratio from my write-up... to encourage degree variance  #############################################################################################
        entry: dict, 
    ...
    """
    passage_text = entry["text"]
    iq_prompt_filled = iq_prompt_template.replace("{{PASSAGE}}", passage_text)
    oq_prompt_filled = oq_prompt_template.replace("{{PASSAGE}}", passage_text)
    ...
    entry["IQs"] = [q.strip() for q in iq_response.split("\n") if q.strip()]
    entry["OQs"] = [q.strip() for q in oq_response.split("\n") if q.strip()]
...
for entry in data:
    updated = generate_iqoq(entry, IQ_prompt, OQ_prompt, MODEL_SERVERS[0])
    if updated:
...
#   "IQs": ["1. Who was Abraham Lincoln's father?", "Who is Thomas Lincoln?", "No question generated"], # file is updated to add these - uncleaned iqoq
#   "OQs": ["- Who was the maternal grandfather of Abraham Lincoln?", "* none", "What is this?"], # file is updated to add these - uncleaned iqoq
#   "generation_model": "Qwen-7b" # file is updated to add these
# }
#
#
...
def clean_iqoq(questions: list[str]) -> list[str]:
    ...
    for q in questions:
        ...
        if len(q) < 5:
            continue                             # skip tiny junk - 5 chars really????
...
def multi_hop_graph_traverse_llm( # helper for run_dev_set()                 ################## I COULD DEFINE THE TWO ALGORITHMS SEPARATELY AND SEND TO THIS AS AN ARG????
    graph: nx.DiGraph, # build_edges() -> build_networkx_graph()
    query_text: str, # from {dataset}_dev/test.jsonl files 
    seed_passages: list, # ids for top-k FAISS passages - from {dataset}_passages.jsonl - and also the jaccard stuff? 
    n_hops: int = 2, # from configs at start
    model_servers: list = None # from configs at start 
):
    """

    THE ACTUAL TRAVERAL ALGORITHM - I NEED TO MAKE THE NORMAL HOPRAG VARIATION OF THIS 

    LLM-driven HopRAG traversal with multi-visit policy.
    - Repeat visits logged per edge.
    - None (skip) edges are not added to edges_chosen, but counted.
    """
    Cqueue = seed_passages[:] # initialise traversal queue of fiass topk passage IDs
    ...
    for hop in range(n_hops):
        ...
        for vj in Cqueue: # for each vertex in Cqueue
            if vj not in graph: 
                print(f"[WARN] Passage '{vj}' from FAISS not found in graph. Skipping.") # log if a FAISS retrieved vertex isn't in the graph
                continue

            # Candidate edges
            candidates = [
                (vk, graph[vj][vk]) # for each edge between two vertices
                for vk in graph.successors(vj) # for each coming from vj?
                if (graph[vj][vk]["oq_id"], graph[vj][vk]["iq_id"]) not in Evisited # if the iq/oq edges are not already in the graph? 
            ]
            if not candidates: # if there are no appropriate edges, just continue anyway 
                continue
            ...
            # If None: skip expansion but count it
            if chosen is None:
            ...
            Evisited.add((chosen_edge["oq_id"], chosen_edge["iq_id"]))
            ...
            if not is_repeat:
                visited_passages.add(chosen_vk)
            ...
            # Always enqueue for multi-visit traversal
    ...
    return list(visited_passages), ccount, hop_trace, {
        ...
        "repeat_visit_count": total_repeat_count
    }
...
def run_dev_set(
    ...
    for entry in query_data:
        ...
        # --- Traverse ---
        visited_passages, ccount, hop_trace, stats = multi_hop_graph_traverse_llm( 
            graph=graph,
            query_text=query_text,
...
def ask_llm_with_passages( ####################################### make this prompt separate + maybe make the name better?
    query_text: str,
    passage_ids: List[str],
    graph: nx.DiGraph, ############## is this querying the DiGraph just for convenience, to find the text to send the model????
    server_url: str,
    max_tokens: int = 100
) -> Dict[str, str]:
    """
    Format top passages and send them with the query to the LLM.
    Returns both raw and normalized LLM output.
    """
    passage_texts = [
        f"[{pid}]: {graph.nodes[pid].get('text', '')}"
