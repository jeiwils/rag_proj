...
Outputs
-------
``results/dense_rag_results.jsonl.gz``
    Predictions generated by :func:`run_dense_rag_baseline`.
``results/dev_results.jsonl.gz`` ######################### why's this called dev? 
...
Example line from ``results/dense_rag_results.jsonl.gz``::
...
``results/dev_results.jsonl.gz`` ######################### what does this look like? 
...
def run_dense_rag_baseline(
    ...
    output_path="results/dense_rag_results.jsonl.gz",
    seed_top_k=50,
    alpha=0.5
):
    """
...
if __name__ == "__main__":
    ...
    run_pipeline(
        ...
        emb_model=emb_model,
        server_configs=server_configs,
        output_path=os.path.join(output_base, "dev_dense.jsonl.gz"),
        seed_top_k=seed_top_k,
        alpha=alpha
    )
    ...
    run_pipeline(
        ...
        emb_model=emb_model,
        server_configs=server_configs,
        output_path=os.path.join(output_base, "dev_hoprag.jsonl.gz"),
        seed_top_k=seed_top_k,
        alpha=alpha,
        n_hops=n_hops
    )
    ...
    run_pipeline(
        ...
        emb_model=emb_model,
        server_configs=server_configs,
        output_path=os.path.join(output_base, "dev_enhanced.jsonl.gz"),
        seed_top_k=seed_top_k,
        alpha=alpha,
        n_hops=n_hops
    )
